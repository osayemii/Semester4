...
@app.route('/library/<login>/<user_id>')
def fun_library(login, user_id):
    if login == 'guest':
        return redirect(url_for('fun_guest', gid=user_id))
    elif login == 'student':
        return redirect(url_for('fun_student', sid=user_id))
    elif login == 'faculty':
        return redirect(url_for('fun_faculty', fid=user_id))
    else:
